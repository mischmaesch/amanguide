...
* [`src/cljs/amanguide_web/`](src/cljs/amanguide_web/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/amanguide_web/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `amanguide-web` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/amanguide_web/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns amanguide-web.example
  (:require [amanguide-web.config :as config])
